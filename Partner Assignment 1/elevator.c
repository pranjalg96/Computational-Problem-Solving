...
4. For scenario 3, only average time per passenger is used in determining best maxFloor1.
	...
	for (int floor1 = min_floor_1; floor1 <= max_floor_1; floor1++)
	{
		...
		for(int floor2 = min_floor_2; floor2 <= max_floor_2; floor2++)
		{
			...
			for(int i = 0; i < num_iterations; i++)
			{
				...
				int max_elev_time = max_ele_time_3(elevator_1.total_time, elevator_2.total_time, elevator_3.total_time); // Get max time of the two elevators
				avg_time += max_elev_time;
				
				double avg_time_pass = (elevator_1.avg_time_per_passenger*elevator_1.num_passengers + elevator_2.avg_time_per_passenger*elevator_2.num_passengers + elevator_3.avg_time_per_passenger*elevator_3.num_passengers)/(elevator_1.num_passengers + elevator_2.num_passengers + elevator_3.num_passengers); // Assumption 3
